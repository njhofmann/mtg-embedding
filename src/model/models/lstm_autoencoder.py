...
class LSTMAutoencoder(a.Seq2SeqAutoencoder):
    ...
    def init_encoder(self) -> Tuple[m.Model, ly.Input]:
        ...
        embedded_inputs = l.Embedding(input_dim=self.vocab_size, output_dim=self.input_embedding_len,
                                      input_length=self.sent_len, name='encoder-embedding', mask_zero=False)(encoder_inputs)
        ...
        encoder = m.Model(inputs=encoder_inputs, outputs=state_h, name='encoder-model')(encoder_inputs)
        return encoder, encoder_inputs

    def init_decoder(self) -> Tuple[m.Model, ly.Layer]:
